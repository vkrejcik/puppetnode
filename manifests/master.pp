class puppetnode::master {
  ...
  exec { 'install-collection':
    command => 'wget https://apt.puppetlabs.com/puppetlabs-release-pc1-jessie.deb;dpkg -i puppetlabs-release-pc1-jessie.deb',
    user    => 'root',
...
# }
  ...
  package { ['ruby', 'ruby-dev', 'build-essential', 'sudo']:
    ensure => 'latest'
  }
